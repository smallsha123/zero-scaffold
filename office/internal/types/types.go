...
type OfficeSearch struct {
	PageReq
	Department int    `form:"department,optional"` //客服部门
	TypeApp    int8   `form:"typeApp,optional"`    //1.微信公众号2.小程序
	IsUsed     uint8  `form:"isUsed,optional"`     //1.可用2.不可用
	Gid        uint   `form:"gid,optional"`         //微信分组ID
	Appid      string `form:"appid,optional"`      //appid
	Platform   uint8  `form:"platform,optional"`    //服务商ID
}
...
type PageReq struct {
	...
	PageSize int64 `json:"page_size,default=20,range=[1:100]" form:"page_size,default=10,range=[1:100]"`
}
